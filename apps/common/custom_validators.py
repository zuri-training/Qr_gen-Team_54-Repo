...
def validate_pdf_file_type(input):
    ...
    if file_type not in pdf_file_types:
        raise ValidationError("Upload Error, File must be a pdf file, please try again.")
...
def validate_mp3_file_type(input):
    ...
    if file_type not in audio_file_types:
        raise ValidationError("Upload Error, file must be an mp3 file, please try again.")
...
def validate_image_file_type(input):
    ...
    if file_type not in image_file_types:
        raise ValidationError("Upload Error, file must be an image file, please try again.")
...
def validate_video_file(input):
    video_file_types = ["video/mp4", "video/webdl", "video/avi", "video/wmv", "video/mkv"]

    if not input:
        raise ValidationError("No file selected, please select a file...")

    if input.size > settings.MAX_VIDEO_FILE_SIZE:
        raise ValidationError(f"Upload Error, file size is {round(input.size/(1024**2),1)}MB and cannot be larger than 100MB.")
    ...
    if file_type not in video_file_types:
        raise ValidationError("Upload Error, File must be an video file, please try again.")
